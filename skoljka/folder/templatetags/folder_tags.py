...
def _descriptors_to_folders(descriptors):
    ...
    folders = Folder.objects.filter(
        cache_tags__in=descriptors, hidden=False, author_id=1
    )
...
def folder_inline_year_shortcuts(context, folder_descriptors, split=1000):
    ...
    school_class = user and user.is_authenticated() and user.get_profile().school_class

    chosen_tag = school_class and next(
        (x[2] for x in settings.USERPROFILE_SCHOOL_CLASS_INFO if x[0] == school_class),
        None,
    )
    ...
    for x in all_folders:
        ...
        any, any_non_rated, percent, RGB = Folder.parse_solution_stats(
            stats and stats[2], solvable_task_count
        )
    ...
    return {
        'main_folder_chunks': [
            main_folders[i : i + split] for i in range(0, len(main_folders), split)
        ],
    }
