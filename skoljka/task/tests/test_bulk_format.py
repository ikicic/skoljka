...
from skoljka.permissions.constants import DELETE, EDIT, MODEL_DEFAULT, VIEW
from skoljka.task import bulk_format
from skoljka.task.bulk_format import (
    parse_add_perm,
    parse_bulk,
    parse_variables,
    split_expression,
...
class TaskBulkFormatTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.alice_folder = Folder.objects.create(
            name="alice's folder", short_name="alice's folder", author=self.alice
        )
    ...
    def assertParseRaises(self, input, exception):
        self.assertRaises(exception, parse_bulk, self.alice, dedent(input))
    ...
    def test_internal_split_expression(self):
        ...
        self.assertRaises(
            bulk_format.ParseError, split_expression, "missing a brace @{foo"
        )
        self.assertEqual(
            split_expression("not missing a brace \\@{foo"),
    ...
    def test_internal_parse_variables(self):
        ...
        self.assertEqual(
            parse_variables({"FOO": "asdf", "BAR": "qwerty"}),
            {"FOO": "asdf", "BAR": "qwerty"},
        )

        # With dependencies.
        self.assertEqual(
            parse_variables({"FOO": "bla@{bar}boo", "BAR": "qwerty"}),
        ...
        self.assertEqual(
            ...
            {
                "A": "blatwo xzzy variables zzboo",
                "B": "two xzzy variables zz",
                "C": "xzzy",
                "D": "zz",
            },
        )
        ...
        # Case-insensitivity
        self.assertEqual(
            parse_variables({"ASDF": "bla @{BaNana}", "BANANA": "bnna"}),
        ...
        self.assertRaises(
            bulk_format.CyclicDependency, parse_variables, {"A": "@{B}", "B": "@{A}"}
        )
