...
def prepare_action_data(context, A):
    ...
    if ttype == TASK_ADD:
        ...
        S = u'je postavi%s službeno %s zadatka' % (G1, A.A('solution', u'rješenje'))
        ...
        if content_type.app_label == 'task' and content_type.model == 'task':
            ...
            try:
                (
                    solution_author_id,
                    solution_author_username,
                    task_id,
                    task_name,
                    task_author_id,
                ) = A.target_cache.split(POST_SEND_CACHE_SEPARATOR)
            except ValueError:
                (
                    solution_author_id,
                    solution_author_username,
            ...
            if user.id == int(solution_author_id):
                ...
                whose_solution = u'%%s korisnika <a href="/profile/%s/">%s</a>' % (
                    solution_author_id,
                    solution_author_username,
                )

            object = u'%s za%s zadatak %s' % (
                whose_solution % solution,
                whose_task,
                task,
            )
        ...
        if ttype == POST_SEND:
            if (
                ...
                and user.get_profile().private_group_id == A.group_id
            ):
                A._label = ('label-info', 'Odgovor')
                S = (
                    u'je <a href="/%s/%d/#post%d">odgovori%s na tvoj komentar</a> na %s:'
...
def activity_list(context, exclude_user=None, target=None, action_object=None):
    ...
    activity = get_recent_activities(
        user, 30, exclude_user=exclude_user, target=target, action_object=action_object
    )
