...
class TeamScoreTest(TestCase):
    ...
    def setUp(self):
        ...
        self.team = Team.objects.create(
            name="Test team", author=self.alice, competition=self.competition
        )
        self.chain = Chain.objects.create(
            competition=self.competition, name="Test chain", bonus_score=1000
        )
    ...
    def simulate_submissions(self, submissions):
        ...
        for index, test_case in enumerate(submissions):
            try:
                ...
                old_chain_submissions = list(
                    Submission.objects.filter(ctask__chain=self.chain)
                )
                ...
                score_triple = (
                    team.cache_score_before_freeze,
                    team.cache_score,
...
class TestCompetitionTaskComments(TestCase):
    ...
    def setUp(self):
        ...
        self.chain = Chain.objects.create(
            competition=self.competition, name="Test chain"
        )
    ...
    def test_update_chain_comments_cache(self):
        ...
        ctasks.append(
            create_ctask(
                self.alice, self.competition, self.chain, "42", 1, "nothing important"
            )
        )
        ctasks.append(
            create_ctask(
...
class ChainsTest(TestCase):
    ...
    def setUp(self):
        ...
        self.chain1 = Chain.objects.create(
            competition=self.competition, name="Test chain 1", bonus_score=1000
        )
        self.chain2 = Chain.objects.create(
            competition=self.competition, name="Test chain 2", bonus_score=1000
        )
    ...
    def test_update_ctask_cache_admin_solved_count(self):
        ...
        Submission.objects.create(
            ctask=ctask, team=team, result="42", score=ctask.max_score
        )
        ...
        # Check if saved.
        self.assertEqual(
            CompetitionTask.objects.get(id=c1.id).cache_admin_solved_count, 1
        )
        self.assertEqual(
            CompetitionTask.objects.get(id=c2.id).cache_admin_solved_count, 0
        )
    ...
    def test_refresh_ctask_cache_admin_solved_count(self):
        ...
        self.assertEqual(
            CompetitionTask.objects.get(id=c1.id).cache_admin_solved_count, 2
        )
        self.assertEqual(
            CompetitionTask.objects.get(id=c2.id).cache_admin_solved_count, 1
        )
        self.assertEqual(
            CompetitionTask.objects.get(id=c3.id).cache_admin_solved_count, 0
        )
    ...
    def test_refresh_update_chain_cache_is_verified(self):
        ...
        self.assertEqual(
            refresh_chain_cache_is_verified(self.competition),
